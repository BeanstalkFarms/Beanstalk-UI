            ...
            >
              ...
              <Box sx={{ width: '100%', mt: 0 }}>
                <TxnAccordion defaultExpanded={false}>
                  ...
                  <Box>
                    <Typography>
                      Note: The amount of FERT received rounds down to the
                      nearest USDC. {usdc?.toFixed(2)} USDC = {fert?.toFixed(0)}{' '}
                      FERT.
                    </Typography>
                  </Box>
                </TxnAccordion>
...
const Buy: React.FC<{}> = () => {
  ...
  const handleQuote = useCallback<QuoteHandler>(
    async (_tokenIn, _amountIn, _tokenOut) => {
      if (!isReplanted) {
        return fertContract.callStatic
          .getUsdcOut(_tokenIn.stringify(_amountIn))
    ...
    [Usdc, Usdt, Weth, fertContract, farm, isReplanted]
  );
    ...
    async (
      ...
      try {
        ...
        if (isReplanted) {
          ...
          const minLP =
            await farm.contracts.curve.zap.callStatic.calc_token_amount(
          ...
          switch (token) {
            case Eth: {
              ...
              const gas = await beanstalk.estimateGas.farm(data, {
            ...
            case Usdc: {
              console.debug('[PurchaseForm] purchasing with USDC', {
                ...
                beanAmount: toStringBaseUnitBN(
                  amountUsdc.times(0.866616),
                  Usdc.decimals
                ),
