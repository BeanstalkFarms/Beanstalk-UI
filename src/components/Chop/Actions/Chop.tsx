...
> = ({
  ...
  const tokenOutputMap = {
    [getChainConstant(UNRIPE_BEAN, chainId).address]: getChainConstant(
      BEAN,
      chainId
    ),
    [getChainConstant(UNRIPE_BEAN_CRV3, chainId).address]: getChainConstant(
      BEAN_CRV3_LP,
      chainId
    ),
  };
  ...
  const penalties = useSelector<AppState, AppState['_bean']['unripe']>(
        ...
        {tokenBalance?.gt(0) ? (
          <>
            ...
            <Stack direction="row" justifyContent="space-between" sx={{ p: 1 }}>
              ...
              <Typography variant="body1" color={BeanstalkPalette.washedRed}>
                {displayBN(displayPenalty)}%
              </Typography>
            </Stack>
            <TokenOutputField token={outputToken} amount={amountOut} />
            <Box>
                    ...
                    actions={[
                      {
                        ...
                        message: `Chop ${displayBN(values.amount)} Unripe ${
                          values.tokens[0].token.symbol
                        }`,
                      },
                      {
                        type: ActionType.BASE,
                        message: `Receive ${displayBN(
                          amountOut
                        )} ${outputToken}`,
                      },
                    ]}
...
const Chop: React.FC<{}> = () => {
  ...
  const onSubmit = useCallback(
    async (
      ...
      let txToast;
      try {
        if (!farmerBalances[urBean.address]?.total.gt(0))
  ...
  return (
    <Formik<ChopFormValues> initialValues={initialValues} onSubmit={onSubmit}>
