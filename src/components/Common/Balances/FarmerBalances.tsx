...
import useFarmerBalancesBreakdown, { StateID, STATE_CONFIG, STATE_IDS } from '~/hooks/farmer/useFarmerBalancesBreakdown';
...
const FarmerBalances: FC<{
  ...
  //
  const assetLabel = 'Balance';
  const availableStates = Object.keys(breakdown.states) as StateID[];
        ...
        <Stack px={{ xs: 0, md: 1 }} py={1} onMouseLeave={onMouseOutContainer} onBlur={onMouseOutContainer}>
          {availableStates.map((id) => (
            <TokenRow
              ...
              onClick={onClick(id)}
              labelTooltip={STATE_CONFIG[id][2]}
            />
          ))}
        </Stack>
      </Grid>
          ...
          <Stack gap={1}>
            ...
            <Box>
              {pieChartData.map((dp) => {
                ...
                const inThisState = breakdown.states[hoverState]?.byToken[token.address];
                return (
                  <TokenRow
                    key={token.address}
                    label={token.symbol}
