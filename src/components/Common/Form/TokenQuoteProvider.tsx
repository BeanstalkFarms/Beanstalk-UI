...
type TokenQuoteProviderProps = TokenQuoteProviderCustomProps &
...
const TokenQuoteProvider: React.FC<TokenQuoteProviderProps> = ({
  ...
  const [result, quoting, getAmountOut] = useQuote(
    tokenOut,
    handleQuote,
    quoteSettings
  );
  ...
  useEffect(() => {
    if (state.token !== tokenOut) {
      console.debug(
        `[TokenQuoteProvider] getting amount out: ${state.amount} ${state.token.symbol} => X ${tokenOut.symbol}`
      );
      getAmountOut(
        state.token, // tokenIn
  ...
  useEffect(() => {
    console.debug(
      `[TokenQuoteProvider] update ${name}.amountOut =>`,
  ...
  const InputProps = useMemo(
    ...
    [state.token, showTokenSelect, isSubmitting, disableTokenSelect]
  );
        ...
        <Stack direction="row" alignItems="center" gap={0.5}>
          {state.amountOut && (
            <Typography variant="body1">
              ≈ {displayFullBN(state.amountOut, tokenOut.displayDecimals)}{' '}
              {tokenOut.symbol}
            </Typography>
          )}
          {quoting && (
