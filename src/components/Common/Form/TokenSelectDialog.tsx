  ...
  ({
    ...
    useEffect(() => {
      if (open) {
        ...
        setNewSelection(new Set(selected.map(({ token }) => token)));
      }
    }, [open, selected]);
    ...
    return (
      ...
      >
        ...
        <StyledDialogContent
          sx={{ padding: 1, pb: mode === TokenSelectMode.MULTI ? 0 : 2 }}
        >
          <List sx={{ p: 0 }}>
            <Stack gap={1}>
              {tokenList
                      ...
                      secondaryAction={
                        balances ? (
                          <Typography variant="bodyLarge">
                            {displayBN(
                              balances?.[_token.address]?.total || ZERO_BN
                            )}
                          </Typography>
                        ) : null
                      }
          ...
          <StyledDialogActions sx={{ pb: 2 }}>
            ...
            >
              {newSelection.size === 0
                ...
                : `Select ${newSelection.size} Token${
                    newSelection.size === 1 ? '' : 's'
                  }`}
            </Button>
          </StyledDialogActions>
        )}
