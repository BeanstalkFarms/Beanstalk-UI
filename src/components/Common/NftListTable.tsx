            ...
            {Object.keys(props.crates)
              ...
              .map((index) => (
                <TableRow key={index}>
                  ...
                  <TableCell align='center' className={classes.lucidaStyle} component='th' scope='index'>
                  {props.indexType === 'time' ? props.crates[index].timeSinceSunrise : props.crates[index].id}
                  </TableCell>
                  {props.crates[index].beans !== undefined
                    ? <TableCell align='center' className={classes.lucidaStyle}>
                      <FormatTooltip
                        placement='right'
                        title={`${displayFullBN(new BigNumber(props.crates[index].beans))} Beans`}
                      >
                        <span>{displayBN(new BigNumber(props.crates[index].beans))}</span>
                      </FormatTooltip>
                    </TableCell>
                    : null
                  }
                  {props.crates[index].txn !== undefined && props.crates[index].txn.length > 2
                    ? <TableCell align='center' className={classes.lucidaStyle}>
                        <Link href={`${BASE_ETHERSCAN_TX_LINK}${props.crates[index].txn}`} color='inherit' target='blank'>
                          <span>{`${props.crates[index].txn.substring(0, 6)}...${props.crates[index].txn.substring(props.crates[index].txn.length - 4)}`}</span>
                        </Link>
                      </TableCell>
                  ...
                  {props.crates[index].account !== undefined && props.assetType !== 'nft'
                    ? <TableCell align='center' className={classes.lucidaStyle}>
                        <Link href={`${BASE_ETHERSCAN_ADDR_LINK}${props.crates[index].account}`} color='inherit' target='blank'>
                          <span>{`${props.crates[index].account.substring(0, 6)}...${props.crates[index].account.substring(props.crates[index].account.length - 4)}`}</span>
                        </Link>
                      </TableCell>
                    : null
                  }
