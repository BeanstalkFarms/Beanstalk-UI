...
const STATE_CONFIG: {
  [key: string]: [name: string, color: string, tooltip: string];
} = {
  // Silo
  deposited: [
    'Deposited',
  ...
  withdrawn: [
    'Withdrawn',
    'rgba(31, 120, 180, 0.3)',
    'Assets being Withdrawn from the Silo. Once the Withdrawal timer elapses, Withdrawn assets become Claimable.',
  ],
  claimable: [
    'Claimable',
    'rgba(178, 223, 138, 0.3)',
    'Assets that can be claimed to your wallet, Deposited in the Silo, etc.',
  ],
  // Farm
  farm: [
    'Farm',
    'rgba(25, 135, 59, 0.5)',
    'Assets stored in Beanstalk but not Deposited.',
  ],
  circulating: [
    'Circulating',
    'rgba(25, 135, 59, 1)',
...
const SiloBalances: React.FC<{
  ...
  const onMouseOver = useCallback(
    (v: StateID) => (allowNewHoverState ? () => setHoverState(v) : undefined),
    [allowNewHoverState]
  );
  ...
  const pieChartData = useMemo(() => {
    if (hoverState) {
      const thisState =
        breakdown.states[hoverState as keyof typeof breakdown.states];
      return Object.keys(thisState.byToken)
        .reduce<PieDataPoint[]>((prev, addr, index) => {
    ...
    return availableStates.map(
      (id) =>
        ({
          ...
          value:
            breakdown.states[
              id as keyof typeof breakdown.states
            ].value.toNumber(),
          color: STATE_CONFIG[id][1],
        } as PieDataPoint)
    );
  }, [hoverState, whitelist, breakdown, availableStates]);
          ...
          >
            {availableStates.map((id) => (
              <TokenRow
                ...
                value={displayUSD(
                  breakdown.states[id as keyof typeof breakdown.states].value
                )}
        ...
        <Grid item xs={12} md={4}>
          {hoverState === null ? (
            ...
            <Stack gap={1}>
              ...
              >
                {STATE_CONFIG[hoverState][0]} Assets
              </Typography>
              <Box>
                {pieChartData.map((dp) => {
                  ...
                  const inThisState =
                    breakdown.states[
                      hoverState as keyof typeof breakdown.states
                    ]?.byToken[token.address];
                  return (
                    <TokenRow
                      key={token.address}
                      label={`${token.name}`}
                      ...
                      tooltip={
                        <>
                          ...
                          <br />≈ {displayUSD(inThisState?.value)}
                        </>
                      }
                    />
                  );
