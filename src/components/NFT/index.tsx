...
export default function NFTs() {
  const { unclaimedNFTs, claimedNFTs } = useSelector<
    AppState,
    AppState['nfts']
  >((state) => state.nfts);
  ...
  const description = (
    <>
      ...
      <span>
        ...
        <Link href={OPENSEA_LINK} target="blank" style={{ color: 'white' }}>
          OpenSea
        </Link>
        .&nbsp;
        <Link
      ...
      <span>
        ...
        From Season 3300 to 3900, up to 2,000 BeaNFTs can be earned by
        participating in Beanstalk. The top 5 largest bean-denominated
        investments each Season (across the Silo and Field) will be awarded one
        of the 2,000 Winter BeaNFTs, until there are none left.{' '}
        <Link
