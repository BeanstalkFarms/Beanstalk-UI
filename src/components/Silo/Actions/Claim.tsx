...
> = ({
  ...
  const handleQuote = useCallback<QuoteHandler>(
    ...
    [farm, token.isLP, pool, values.destination]
  );
                      ...
                      actions={[
                        {
                          ...
                          message: `Claim ${displayTokenAmount(
                            amount,
                            token
                          )}.`,
                        },
                        {
                          ...
                          message:
                            values.tokenOut === token
                              ? `Unpack ${displayTokenAmount(
                                  amount,
                                  token
                                )} into 3CRV and send ${destination}.`
                              : `Send ${displayTokenAmount(
                                  amount,
                                  token
                                )} and send ${destination}.`,
                        },
                      ]}
                    />
                  </AccordionDetails>
    ...
    async (
      ...
      try {
        ...
        if (!crates || crates.length === 0)
          throw new Error('No claimable crates');

        txToast = new TransactionToast({
          loading: `Claiming ${displayFullBN(claimableBalance)} ${
            token.name
          } from the Silo`,
        ...
        if (token.isLP && removeLiquidity) {
          ...
          values?.token?.steps.forEach((step, i) =>
            console.debug(`step ${i}:`, step.decode(encoded[i]))
          );
          data.push(...encoded);
        }
