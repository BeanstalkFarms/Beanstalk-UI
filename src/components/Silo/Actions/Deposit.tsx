      ...
      <Form noValidate autoComplete="off">
        ...
        <Stack gap={1}>
          ...
          {isReady ? (
            <>
              <TxnSeparator />
              <TokenOutputField token={siloToken} amount={bdv} />
              <Stack direction="row" gap={1} justifyContent="center">
                <Box sx={{ flex: 1 }}>
                  <TokenOutputField token={STALK} amount={stalk} />
                </Box>
                <Box sx={{ flex: 1 }}>
                  <TokenOutputField token={SEEDS} amount={seeds} />
                </Box>
              </Stack>
              <Box>
...
const Deposit: React.FC<{
  ...
  const isUnripe =
    whitelistedToken === urBean || whitelistedToken === urBeanCrv3;
  ...
  }, [isUnripe, whitelistedToken, allAvailableTokens]);
  ...
  const handleQuote = useCallback<QuoteHandler>(
    async (_tokenIn, _amountIn, _tokenOut) => {
      ...
      const balanceIn: Balance =
        _tokenIn instanceof NativeToken
      ...
      // Depositing LP Tokens
        ...
        if (!pool)
          throw new Error(
            `Depositing to ${tokenOut.symbol} but no corresponding pool data found.`
          );
        ...
        if (/* pool is Curve MetaPool */ true) {
          ...
          // This is a CRV3-underlying stable (DAI/USDC/USDT etc)
            ...
            if (underlyingTokenIndex === 0)
              throw new Error('Malformatted pool.tokens / pool.underlying');
            const indices = [0, 0, 0];
    ...
    [farm, pool, getChainToken, Weth, balances]
  );
  ...
  const onSubmit = useCallback(
      ...
      try {
        ...
        txToast = new TransactionToast({
          loading: `Depositing ${displayFullBN(
            ...
            whitelistedToken.displayDecimals
          )} ${whitelistedToken.name} to the Silo`,
    ...
    [Eth, beanstalk, whitelistedToken, refetchFarmerSilo]
  );
