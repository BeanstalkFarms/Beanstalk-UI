  ...
  function transactionDetails() {
    ...
    return (
      ...
      <div style={{display: 'inline-block', width: '100%'}}>
        <span>{`You will immediately receive ${displayBN(new BigNumber(toStalkValue))} Stalk, ${displayBN(new BigNumber(toSeedsValue))} Seeds.`}</span>
        <br/>
        <span>{`This currently translates to ${toStalkValue.dividedBy(props.totalStalk).multipliedBy(100).toFixed(3)}% ownership of Beanstalk.`}</span>
      </div>
      </>
    )
  }
