...
export default function Silo(props) {
  ...
  return (
    <ContentSection id='silo' title='Silo'>
      <Box className={"section-description"} style={width > 500 ? {maxWidth: '550px', margin: '20px 0 0 0', padding: '12px'} : {width: width - 64, margin: '20px 0', padding: '12px'}}>
        ...
        {` `}<Link href={`${MEDIUM_INTEREST_LINK}#8b79`} target='blank'>{'Read More'}</Link>{`.`}
      </Box>
      <TabbedSilo {...props} />
    </ContentSection>
  )
