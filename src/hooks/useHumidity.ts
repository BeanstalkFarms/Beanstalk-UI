...
export const useHumidityAtSeason = () => {
  ...
  return useCallback(
    (season: BigNumber) => {
      ...
      const seasonsAfterReplant = MaxBN(
        season.minus(replantSeason.plus(1)),
        ZERO_BN
      );
      if (season.lte(replantSeason))
...
export default function useHumidity() {
  const season = useSelector<AppState, AppState['_beanstalk']['sun']['season']>(
