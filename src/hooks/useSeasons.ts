...
type MinimumViableSeason = Partial<Season> &
  Pick<Season, 'id' | 'seasonInt' | 'timestamp'>;

const useSeasons = <T extends { seasons: MinimumViableSeason[] }>(
  ...
  useEffect(() => {
    (async () => {
      console.debug(
        `[useRecentSeasonsData] initializing with range = ${range}`
      );
      try {
        if (range !== SeasonRange.ALL) {
          ...
          console.debug(
            `[useRecentSeasonsData] requested all seasons. current season is ${latestSubgraphSeason}. oldest loaded season ${null}`,
            init.data.seasons
          );
          ...
          const promises = [];
          console.debug(
            `[useRecentSeasonsData] needs ${numQueries} calls to get ${latestSubgraphSeason} more seasons`
          );
          setLoading(true);
          for (let i = 0; i < numQueries; i += 1) {
            ...
            console.debug(
              `[useRecentSeasonsData] get: ${season} -> ${Math.max(
                season - 1000,
                2
              )}`
            );
