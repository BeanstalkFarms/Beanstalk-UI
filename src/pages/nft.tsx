...
const NFTPage: React.FC = () => {
  ...
  const parseMints = useCallback(
    (accountNFTs: any, contractAddress: string, setNFTs: any) => {
      ...
      fetch(fetchURL, requestOptions)
        ...
        .then((response) => {
          ...
          const nftHashes = response.ownedNfts.map((nft: any) =>
  ...
  const mintAllGenesis = () => {
    if (
      ...
      const accounts = Array(unmintedGenesis.length).fill(
        getAccount(account.address)
      );
  ...
  const mintAllWinter = () => {
    if (
      ...
      winterContract
        .batchMintAccount(getAccount(account.address), tokenIds, signatures)
        .then((txn) => {
          txToast.confirming(txn);
          return txn.wait();
        })
  ...
  return (
    <Container maxWidth="lg">
      <Stack spacing={2}>
        <PageHeader title="BeaNFTs" description="View and mint your BeaNFTs" />
        <Card sx={{ p: 2 }}>
          <Stack gap={1}>
            ...
            >
              ...
              {tab === 0 && genesisNFTs && (
                <Button
                  disabled={!unmintedGenesis || unmintedGenesis.length === 0}
              ...
              {tab === 1 && winterNFTs && (
                <Button
                  disabled={!unmintedWinter || unmintedWinter.length === 0}
            ...
            {/* genesis */}
            {tab === 0 && (
              <NFTGrid nfts={genesisNFTs} handleDialogOpen={handleDialogOpen} />
            )}
            {/* winter */}
            {tab === 1 && (
              <NFTGrid nfts={winterNFTs} handleDialogOpen={handleDialogOpen} />
            )}
          </Stack>
        </Card>
      </Stack>
