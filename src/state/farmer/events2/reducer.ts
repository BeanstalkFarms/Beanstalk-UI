    ...
    .addCase(ingestEvents, (state, { payload }) => {
      ...
      if (!state[id]) {
        ...
        if (
          ...
          payload.startBlockNumber <= c.endBlockNumber
        ) {
          console.warn(
            `[events2] Cache ID ${payload.cache} previously searched for events through ${c.endBlockNumber}, but received a new payload with blocks starting at ${payload.startBlockNumber}. There may be duplicates.`
          );
        }
        state[id] = {
          ...c,
          events: [...c.events, ...payload.events],
          endBlockNumber: payload.endBlockNumber,
          updatedAt: payload.timestamp,
        };
      }
