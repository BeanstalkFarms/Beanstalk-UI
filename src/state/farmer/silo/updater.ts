...
import useEventProcessor, {
  EventParsingParameters,
} from 'hooks/useEventProcessor';
...
export const useFetchFarmerSilo = () => {
  ...
  const getQueryFilters = useCallback<GetQueryFilters>(
    (_account, fromBlock, toBlock) =>
      migrate<Beanstalk, BeanstalkReplanted, Promise<ethers.Event[]>[]>(
        beanstalk,
        [
          (b) => [
            // Silo (v1)
            ...
            b.queryFilter(
              b.filters['ClaimSeasons(address,address,uint32[],uint256)'](
                _account
              ),
              fromBlock || blocks.BEANSTALK_GENESIS_BLOCK,
              toBlock || 'latest'
        ...
        dispatch(
          updateFarmerSiloBalances({
            [SiloTokens.Bean.address]: {
              deposited: Object.keys(results.userBeanDeposits).reduce(
                (prev, s) => {
              ...
              ...parseWithdrawals(
                results.beanWithdrawals,
                eventParsingParameters.season
              ),
            },
            [SiloTokens.BeanEthLP.address]: {
              deposited: Object.keys(results.userLPDeposits).reduce(
                (prev, s) => {
              ...
              ...parseWithdrawals(
                results.lpWithdrawals,
                eventParsingParameters.season
              ),
            },
            [SiloTokens.BeanCrv3LP.address]: {
              deposited: Object.keys(results.userCurveDeposits).reduce(
                (prev, s) => {
              ...
              ...parseWithdrawals(
                results.curveWithdrawals,
                eventParsingParameters.season
              ),
            },
            [SiloTokens.BeanLusdLP?.address]: {
              deposited: Object.keys(results.userBeanlusdDeposits).reduce(
                (prev, s) => {
