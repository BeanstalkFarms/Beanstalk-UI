...
import { account, beaNFTContract, beaNFTContractReadOnly, txCallback } from './index'
export const mintNFT = async (account, nftId, ipfsHash, signature) => {
    beaNFTContract().mint(account, nftId, ipfsHash, signature).then(response => {
      response.wait().then(receipt => { txCallback() })
    })
}

export const mintAllNFTs = async (account, nftId, ipfsHash, signature) => {
    beaNFTContract().batchMint(account, nftId, ipfsHash, signature).then(response => {
      response.wait().then(receipt => { txCallback() })
    })
}
...
export const getMintedNFTs = async () => {
    ...
    const toTransfers = await beaNFT.getPastEvents('Transfer', {filter: {to: account},fromBlock:0})
    const fromTransfers = await beaNFT.getPastEvents('Transfer', {filter: {from: account},fromBlock:0})
